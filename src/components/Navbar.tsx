...
export default function Navbar({ lang, dict }: NavbarProps) {
    ...
    return (
        <nav className={`fixed w-full z-50 transition-colors duration-500 py-6 ${showBackground ? 'bg-white/95 shadow-md' : 'bg-transparent'}`}>
            <Container size="xl">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex-shrink-0">
                        <Link href={`/${lang}`} className="flex items-center gap-3 group">
                            ...
                            <span className={`font-display font-medium text-3xl tracking-wide mt-1 ${isDarkText ? 'text-gmrt-blue' : 'text-white'}`}>
                                GMRT
                            </span>
                        </Link>
                    </div>
                    ...
                    <div className={`hidden md:flex items-center space-x-6 ${isDarkText ? 'text-slate-600' : 'text-white/90'}`}>
                        <button onClick={toggleLanguage} className={`cursor-pointer text-lg font-light flex items-center gap-1 ${isDarkText ? 'hover:text-gmrt-salmon' : 'hover:text-gmrt-salmon'}`}>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 w-full bg-white border-t border-slate-100 py-4 px-6 md:hidden shadow-xl">
                    <div className="flex flex-col gap-4">
                        ...
                        <button onClick={() => { toggleLanguage(); setIsOpen(false); }} className="text-2xl py-2 font-light text-slate-600 hover:text-gmrt-blue flex items-center gap-2">
                            <Globe size={24} /> <span>Language: {lang.toUpperCase()}</span>
                        </button>
                    </div>
                </div>
            )}
